...
#SBATCH --cpus-per-task=32     # Ask for full nodes is better (32 graham and Beluga / 48 in Cedar)
...
# Originally Kate's script, but input files were updated after trimming 
# Reference FASTA: the latest updated version is used in this script 
...
TO_STAGE=/home/sfarida/scratch/Mapping_2/Ceratina_fastq_files_subset${SLURM_ARRAY_TASK_ID}
mkdir -p ${STAGED_PATH}
parallel --will-cite -j ${SLURM_CPUS_PER_TASK} cp {} ${STAGED_PATH} ::: ${TO_STAGE}/*.fastq.gz
INPUTS=$(ls ${STAGED_PATH}/*_R1_htrimmed_paired.fastq.gz)
...
do
        PREF="${R1/_R1*}"
        R2="${PREF}_R2_htrimmed_paired.fastq.gz"
        PREF=$(basename ${PREF})
        ...
        then
                ...
                ( time bash -c \
                "bwa mem -t ${SLURM_CPUS_PER_TASK} "${REF}" "${R1}" "${R2}" -M | \
